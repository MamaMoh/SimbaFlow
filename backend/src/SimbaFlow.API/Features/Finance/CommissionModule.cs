...
public class CommissionModule : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/board", async (
            int? page, int? pageSize, string? status, Guid? officeId, string? country, string? search,
            ISender sender) =>
        {
            var result = await sender.Send(new GetCommissionBoardQuery(
                page ?? 1, pageSize ?? 50, status, officeId, country, search));
        ...
        group.MapGet("/reports/by-office", async (
            Guid? officeId, DateOnly? from, DateOnly? to, ISender sender) =>
        {
            var result = await sender.Send(new GetCommissionReportQuery(officeId, from, to));
            return result.IsSuccess ? Results.Ok(result) : Results.Json(result, statusCode: result.StatusCode);
        });
